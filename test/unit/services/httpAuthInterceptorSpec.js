...
define(['mocks', 'services/httpAuthInterceptor'], function () {
  ...
  describe('httpAuthenticator', function () {
    ...
    it('should succeed on 200', function () {
      $httpBackend.whenGET('/order/').respond(200, '');

      $http.get('/order/').
        success(function (data, status, headers, config) {
          pass(data, status, config);
        }).
        error(fail);

      $httpBackend.flush();

      expect(pass).toHaveBeenCalledWith('', 200, { method: 'GET', url: '/order/' });
    ...
    it('should pass through to error when 500', function () {
      ...
      expect(fail).toHaveBeenCalledWith(500, {}, { method: 'GET', url: '/order/' });
    });
  });
});